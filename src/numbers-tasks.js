...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const angleTop = x1 * x2 + y1 * y2;
  const angleBot = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(y2 ** 2 + x2 ** 2);
  const angle = Math.acos(angleTop / angleBot);
  return angle;
}
...
function toFixed(number, fractionDigits) {
  ...
  const fixedNumber = Number(number).toFixed(fractionDigits);
  const trimmedNumber = fixedNumber.replace(/\.?0+$/, '');
...
function isInteger(number) {
  return (
    typeof number === 'number' && Number.isFinite(number) && number % 1 === 0
  );
}
